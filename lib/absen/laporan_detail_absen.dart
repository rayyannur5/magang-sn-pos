...
class LaporanDetailAbsen extends StatefulWidget {
  const LaporanDetailAbsen({super.key, required this.date});
  final DateTime date;

  @override
  State<LaporanDetailAbsen> createState() => _LaporanDetailAbsenState(date);
}
...
class _LaporanDetailAbsenState extends State<LaporanDetailAbsen> {
  ...
  Future dataShift() async {
    List dataAbsen = await Absen().absensiReport(date, date);
  ...
  Future getPenjualan() async {
    var pref = await SharedPreferences.getInstance();
    var idUser = pref.getString('id_user');
    String formattedBeginDate = DateFormat('yyyy-MM-dd').format(date);
    String formattedEndDate = DateFormat('yyyy-MM-dd').format(date);
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Text('Laporan Detail Absen', style: TextStyle(fontFamily: 'Poppins', fontSize: 25, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
          Container(
